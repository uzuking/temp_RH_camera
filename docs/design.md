...
- ESP32 から WiFi 経由で Raspberry Pi に HTTP POST でデータ送信
- Raspberry Pi 側で CSV と JPEG を蓄積
...
| カメラ | Grove Serial Camera Kit（UART、JPEG、3.3V ロジック） | 苗の成長撮影（レベル変換不要、後述） |

### 配線

| 接続元 | 接続先 | 備考 |
|--------|--------|------|
| カメラ VCC (赤) | ESP32 VIN (5V) | USB 給電の 5V 系統から供給 |
| カメラ GND (黒) | ESP32 GND | |
| カメラ TX (黄) | ESP32 GPIO16 (RX2) | 3.3V ロジック、直結OK |
| カメラ RX (白) | ESP32 GPIO17 (TX2) | 直結OK |
...
- カメラは 5V 電源だが信号は 3.3V レベル（TX 実測 2.8V）。3V3 ピンからは供給しない
- DHT22 は 3.3V 駆動
- GND は全デバイスで共通にする
- USB 給電で運用
...
当初はカメラ TX が 5V と想定し抵抗分圧によるレベル変換を計画していたが、
実測の結果 TX 出力は 2.8V（3.3V ロジック）であり、ESP32 GPIO に直結で問題ない。
抵抗分圧を挟むと信号が減衰して通信できなくなるため、**直結すること**。

## 通信方式

ESP32 → Raspberry Pi に HTTP POST。家庭内 LAN 内のみで使用。
...
640x480 JPEG は 12-15KB（実測）。WiFi スタック使用時の ESP32 空き RAM は約 100KB。
→ 640x480 で運用中。64KB バッファで問題なし。
...
ルーターで Raspberry Pi に固定 IP を割り当てるか、mDNS（`seedling-server.local`）を使用。
...
- [x] **3. カメラ UART 通信確認** — 完了条件: カメラからの応答バイトを確認。結果: OV528 SYNC 成功（115200baud、GPIO16/17 直結、レベル変換不要）
- [x] **4. WiFi + NTP 時刻同期** — 完了条件: JST 時刻がシリアルモニタに表示。結果: ntp.nict.jp から JST 取得成功（115200baud WiFi接続）
- [x] **5. 温湿度の HTTP POST** — 完了条件: PC 側でリクエスト受信を確認。結果: DHT22+WiFi+NTP統合、JSON POST で 200 OK 確認
- [x] **6. PC 側受信サーバー** — 完了条件: CSV にデータが追記される。結果: Python(uv) サーバーで sensor_YYYY-MM.csv に自動保存確認
- [x] **7. Grove Camera JPEG 取得** — 完了条件: JPEG データのサイズと先頭バイト (FFD8) を確認。結果: OV528 JPEG取得成功（640x480、FFD8/FFD9確認）
- [x] **8. 画像の HTTP POST** — 完了条件: PC 側で JPEG ファイルが保存される。結果: POST /image で YYYYMMDDTHHMMSS.jpg として保存確認
- [x] **9. 統合 + 定期送信** — 完了条件: 10 分/1 時間の間隔で自動送信が動作。結果: DHT22+カメラ+WiFi統合、millis()ベース定期送信で CSV・JPEG 自動蓄積確認
- [x] **10. 運用準備** — Raspberry Pi にサーバー配置、systemd 自動起動、固定 IP 設定。結果: RPi4(Debian12)にSSH+固定IP(192.168.105.200)+seedling-server(systemd)+画像ビューア(port9090)を配置、ESP32からの定期送信（センサー10分/カメラ1時間）確認済み。DVC設定は後日
...
開発時は PC 上の `~/Documents/My_garden/smart/seedling-server/` で動作確認済み（Python、uv 管理）。
運用環境は Raspberry Pi に配置予定。
